...
import { useCompactFit } from "@/hooks/useCompactFit";
 ...
 *
 * The compact layout only holds while the message fits on one line: as soon as
 * it wraps, the controls drop to their own row and the text takes the whole
 * bubble, so a compact composer stays small at rest without cramping a long
 * message.
 *
...
export function ComposerShell({
    ...
    const fitsCompact = useCompactFit(textareaRef, boxRef, compact, text);
    const compactRow = compact && fitsCompact;
    useFlip(compactRow, boxRef, rowRef);
    ...
    const textareaEl = (
        <textarea
            ...
            className={cn(
                ...
                compactRow ? "max-h-9 flex-1 pr-0" : "order-first w-full basis-full",
            )}
        />
    );
    ...
    return (
        ...
        >
            ...
            <div
                ref={boxRef}
                className={cn(
                    "overflow-hidden rounded-bubble border border-input bg-card/60 shadow-surface backdrop-blur-sm transition-shadow focus-within:border-ring focus-within:ring-2 focus-within:ring-ring/40",
                    compact ? "p-1.5" : "p-2",
                )}
            >
                {reply}
                <div
                    ...
                    className={cn(
                        "flex items-center",
                        compactRow ? "gap-1" : "flex-wrap gap-x-1.5",
                    )}
                >
                    {attachmentMenu}
                    {textareaEl}
                    <div
                        className={cn(
                            ...
                            compactRow ? "gap-1" : "ml-auto gap-1.5",
                        )}
                    >
                        {rightControls}
                        {sendControl}
